...
from .models import Student2, Address2,Product
...
def create_product(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('product_list')  # Redirect to the product list page or wherever needed
    else:
        form = ProductForm()
    
    return render(request, 'usermodule/create_product.html', {'form': form})
...
def product_detail(request, id):
    ...
    return render(request, 'usermodule/product_detail.html', {'product': product})
...
def update_product(request, id):
    ...
    return render(request, 'usermodule/update_product.html', {'form': form, 'product': product})
def delete_product(request, id):
    product = get_object_or_404(Product, id=id)
    if request.method == 'POST':
        product.delete()
        return redirect('product_list')
    return render(request, 'usermodule/delete_product.html', {'product': product})
